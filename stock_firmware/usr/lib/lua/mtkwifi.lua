...
function tableEqual(table1, table2)
    ...
    -- Check if each key-value pair in table1 exists in table2
    for key, value in pairs(table1) do
        repeat
            if type(value) == "table" then
                -- If the value is a table, recursively check it
                if not tableEqual(value, table2[key]) then
                    return false
                end
            elseif type(table2[key]) == "number" and type(value) == "string" and table2[key] == tonumber(value) then
                break
            elseif type(table2[key]) == "string" and type(value) == "number" and tonumber(table2[key]) == value then
                break
            elseif table2[key] ~= value then
                -- If the value is not a table, compare it directly
                return false
        end
        until true
    end

    -- Check if each key-value pair in table2 exists in table1
    for key, value in pairs(table2) do
        repeat
            if type(value) == "table" then
                -- If the value is a table, recursively check it
                if not tableEqual(value, table1[key]) then
                    return false
                end
            elseif type(table1[key]) == "number" and type(value) == "string" and table1[key] == tonumber(value) then
                break
            elseif type(table1[key]) == "string" and type(value) == "number" and tonumber(table1[key]) == value then
                break
...
function mtkwifi.save_profile(cfgs, path)
    ...
    local oldcfgs = mtkwifi.load_profile(path, false)
    if tableEqual(oldcfgs, cfgs) then
        return
    end
...
function mtkwifi.save_read_easymesh_profile(easymesh_cfgs)
    ...
    local oldcfgs = mtkwifi.load_profile(mtkwifi.__read_easymesh_profile_path(), false)
    if tableEqual(oldcfgs, easymesh_cfgs) then
        return
    end
...
function mtkwifi.save_easymesh_mapd_profile(easymesh_mapd_cfgs)
    ...
    local oldcfgs = mtkwifi.load_profile(mtkwifi.__easymesh_mapd_profile_path(), false)
    if tableEqual(oldcfgs, easymesh_mapd_cfgs) then
        return
    end
...
function mtkwifi.save_write_easymesh_profile(easymesh_mapd_cfgs)
    ...
    local oldcfgs = mtkwifi.load_profile(mtkwifi.__write_easymesh_profile_path(), false)
    if tableEqual(oldcfgs, easymesh_mapd_cfgs) then
        return
    end
...
function mtkwifi.get_easymesh_al_mac(devRole)
    ...
    elseif mapd_app_cfgs['al_inf'] then
        almac = c_get_macaddr(mapd_app_cfgs['al_inf'])
        if (almac["macaddr"]  ~= "") then
            r['status'] = 'SUCCESS'
            r['al_mac'] = almac["macaddr"]
        else
            r['status'] = 'FAILED'
        end
    else
...
function mtkwifi.save_easymesh_bss_cfgs(cfgs)
    ...
    local oldcfgs = mtkwifi.load_easymesh_bss_cfgs()
    if tableEqual(oldcfgs, cfgs) then
        return
    end

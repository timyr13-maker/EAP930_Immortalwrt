...
(function($,specialEventName,marginX,marginY){'use strict';var nativeEvent=Object.create(null);var getTime=function(){return new Date().getTime()};nativeEvent.original='click';if('ontouchstart'in document){nativeEvent.start='touchstart';nativeEvent.end='touchend'}else{nativeEvent.start='mousedown';nativeEvent.end='mouseup'}$.event.special[specialEventName]={setup:function(data,namespaces,eventHandle){var $element=$(this);var eventData={};$element.off(nativeEvent.original).on(nativeEvent.original,false).on(nativeEvent.start+' '+nativeEvent.end,function(event){eventData.event=event.originalEvent.changedTouches?event.originalEvent.changedTouches[0]:event}).on(nativeEvent.start,function(event){if(event.which&&event.which!==1){return}eventData.target=event.target;eventData.pageX=eventData.event.pageX;eventData.pageY=eventData.event.pageY;eventData.time=getTime()}).on(nativeEvent.end,function(event){if(eventData.target===event.target&&getTime()-eventData.time<750&&(Math.abs(eventData.pageX-eventData.event.pageX)<=marginX&&Math.abs(eventData.pageY-eventData.event.pageY)<=marginY)){event.type=specialEventName;event.pageX=eventData.event.pageX;event.pageY=eventData.event.pageY;eventHandle.call(this,event);if(!event.isDefaultPrevented()){$element.off(nativeEvent.original).trigger(nativeEvent.original)}}})},remove:function(){$(this).off(nativeEvent.start+' '+nativeEvent.end)}};$.fn[specialEventName]=function(fn){return this[fn?'on':'trigger'](specialEventName,fn)}})(jQuery,'tap',10,10);